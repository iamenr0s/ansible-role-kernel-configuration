...
Thanks for taking the time to contribute to `iamenr0s.ansible_role_kernel_configuration`!
...
Molecule uses Podman locally and Docker in CI. The CI matrix covers Debian, Ubuntu, AlmaLinux, Rocky Linux, and Fedora — changes must not break any supported distro (see `meta/main.yml` for the full platform list).
